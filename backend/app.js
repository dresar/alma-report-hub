...
import_dotenv.default.config({ path: (0, import_node_path.join)(process.cwd(), "../.env") });
import_dotenv.default.config({ path: (0, import_node_path.join)(process.cwd(), ".env") });
var app = (0, import_express.default)();
var PORT = process.env.BACKEND_PORT || process.env.PORT || 3001;
app.use(
...
var sql = (0, import_postgres.default)(databaseUrl, {
  ssl: "require",
  max: 10,
  idle_timeout: 20,
  connect_timeout: 10,
...
var JWT_SECRET = process.env.JWT_SECRET || "sira-fallback-secret";
...
var handleRoute = (fn) => async (req, res, _next) => {
  try {
    await fn(req, res);
  } catch (err) {
...
app.post(
  ...
  handleRoute(async (req, res) => {
    ...
    const ok = await import_bcryptjs.default.compare(oldPassword, rows[0].password_hash);
...
app.post(
  ...
  handleRoute(async (req, res) => {
    ...
    if (fields.bobotTugas !== void 0) {
      updates.push("bobot_tugas");
      values.push(fields.bobotTugas);
    }
    if (fields.bobotUts !== void 0) {
      updates.push("bobot_uts");
      values.push(fields.bobotUts);
    }
    if (fields.bobotUas !== void 0) {
      updates.push("bobot_uas");
      values.push(fields.bobotUas);
    }
    if (fields.sortOrder !== void 0) {
      updates.push("sort_order");
      values.push(fields.sortOrder);
    }
    if (fields.isActive !== void 0) {
...
app.post(
  ...
  handleRoute(async (req, res) => {
    ...
    if (academicYearId || classLevel || rombelId) {
      ...
      return res.json({ data: rows2, total: totalCount2, page: parsedPage, limit: parsedLimit });
    }
    const rows = await sql`
...
app.get("/api/health", (_req, res) => res.json({ status: "ok", time: /* @__PURE__ */ new Date() }));
app.listen(PORT, () => {
  console.log(`\u{1F680} SIRA API Backend running on http://localhost:${PORT}`);
});
